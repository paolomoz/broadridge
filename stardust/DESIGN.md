---
name: Broadridge (migration target — exact replica)
description: Target design system for the 1:1 EDS migration — equals the captured live system verbatim
colors:
  white: "#ffffff"
  navy-deep: "#000c36"
  navy: "#001f5a"
...
# Broadridge — target design system (exact replica)

Target system written by `stardust:direct` under brand-faithful replica mode:
the migration target **equals** the captured live system. Every token below is
inherited from `stardust/current/_brand-extraction.json`; no divergence applied.
Prototype and migrate must reproduce, not reinterpret.
