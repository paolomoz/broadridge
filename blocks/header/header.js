...
export default async function decorate(block) {
  ...
  // Broadridge two-tier chrome: utility links move to a top bar;
  // the strong-wrapped Contact us renders as the primary button in the main row
  ...
  if (navTools) {
    ...
    navTools.querySelectorAll(':scope .default-content-wrapper > ul > li').forEach((li) => {
      if (li.querySelector('strong')) {
        ...
        if (a) a.className = 'button';
        return; // stays in main row
      }
      utilList.append(li);
    });
  ...
  // centered main row container (utility bar stays full-bleed)
  const navMain = document.createElement('div');
  navMain.className = 'nav-main';
  const hamb = nav.querySelector('.nav-hamburger');
  [hamb, nav.querySelector('.nav-brand'), nav.querySelector('.nav-sections'), nav.querySelector('.nav-tools')]
    .filter(Boolean).forEach((el) => navMain.append(el));
  nav.append(navMain);
